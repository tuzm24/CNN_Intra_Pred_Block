        ...
        for idx_data, d in enumerate(self.loader_test):
            for idx_scale, scale in enumerate(self.scale):
                ...
                for lr, hr, ext, filename in tqdm(d, ncols=80):
                    ...
                    #     os.path.join(self.args.dir_demo, '{}/{}{}'.format(
                    #         self.args.tu_data, os.path.splitext(os.path.basename(filename))[0], '.npz'
                    #     ))
                    # )['LUMA'])
                    ...
                    self.ckp.log[-1, idx_data, 0] += prediction.eq(hr).sum().float()/(h*w) * 100.0
                ...
                self.ckp.write_log(
                    '[{}]\tAccuracy: {:.3f} (Best: {:.3f} @epoch {})'.format(
                        d.dataset.name,
